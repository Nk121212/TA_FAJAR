...
const FixingNumberAmount = value => {
  ...
  if (parseInt(splitValue) > 0 && parseInt(splitValue) <= 99) {
    result = `${
      parseInt(value.split(',')[value.split(',').length - 1].split('')[0]) + 1
    }00`;

    if (parseInt(result) >= 1000) {
      result = '000';
      arrResult[arrResult.length - 2] =
        parseInt(arrResult[arrResult.length - 2]) + 1;
    }
  } else {
...
export {ToRupiah, FixingNumberAmount};
